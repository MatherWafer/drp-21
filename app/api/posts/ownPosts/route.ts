    ...
    const posts = await prisma.$queryRaw<PrismaPost[]>(Prisma.sql`
      ...
      ${filterPolygon
        ? Prisma.sql`
            ...
            WHERE p."profileId" = ${userId}::uuid AND ir."profileId" = ${userId}::uuid
          `
        : Prisma.sql`WHERE p."profileId" = ${userId}::uuid`
      }
      ORDER BY ${Prisma.raw(orderBy.replace('createdAt', 'postedOn'))} DESC
