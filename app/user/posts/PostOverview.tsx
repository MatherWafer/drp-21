...
export default function PostOverview({ post }: { post: PostInfo }) {
  ...
  try {
    ...
    if (disliked) {
      const removeDislike = await fetch(`/api/posts/${post.id}/dislike`, {
        method: 'DELETE',
    ...
    // Now toggle like (POST if not liked, DELETE if liked)
...
const handleDislike = async () => {
  try {
    ...
    if (liked) {
      const removeLike = await fetch(`/api/posts/${post.id}/like`, {
        method: 'DELETE',
        body: JSON.stringify({ postId: post.id }),
      });
      if (removeLike.ok) {
    ...
    // Now toggle dislike (POST if not disliked, DELETE if disliked)
